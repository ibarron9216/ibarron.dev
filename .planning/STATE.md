...
last_activity: 2026-05-24 — Deploy script (gh-pages), removed unused JPEG, kept internal files tracked
progress:
  total_phases: 12
  completed_phases: 12
  total_plans: 27
...
Last activity: 2026-05-24 — Deploy script (gh-pages), removed unused JPEG, kept internal files tracked
...
- Set up GitHub Pages to deploy from `gh-pages` branch (run `scripts/deploy-gh-pages.sh` first)

### Decisions

- Internal files (CLAUDE.md, .planning/, docs/, .claude/) kept in git tracking for cross-machine agent use
- Public deployment via `gh-pages` branch created by `scripts/deploy-gh-pages.sh`
- Unused `dr-irving-barron-profile.jpeg` deleted from repo (replaced by .webp)
